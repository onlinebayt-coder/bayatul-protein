...
const AddGamingZoneBrand = () => {
  ...
  const fetchGamingZoneBrand = async (brandId) => {
    setLoading(true)
    try {
      const token = localStorage.getItem("adminToken")
      const { data } = await axios.get(`${config.API_URL}/api/gaming-zone-brands/${brandId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      setFormData({
      ...
      showToast("Failed to load gaming zone brand for editing", "error")
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("adminToken")
      if (isEdit && id) {
        await axios.put(`${config.API_URL}/api/gaming-zone-brands/${id}`, {
        ...
        for (let i = 0; i < formData.brands.length; i++) {
          try {
            await axios.post(`${config.API_URL}/api/gaming-zone-brands`, {
              gamingZonePageSlug: formData.gamingZonePageSlug,
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to remove this brand from the gaming zone page?")) {
      try {
        const token = localStorage.getItem("adminToken")
        await axios.delete(`${config.API_URL}/api/gaming-zone-brands/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        ...
        <div className="p-8">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-bold text-gray-900">
              {isEdit ? "Edit Brand" : "Add Brands to Gaming Zone"}
            </h1>
...
export default AddGamingZoneBrand
