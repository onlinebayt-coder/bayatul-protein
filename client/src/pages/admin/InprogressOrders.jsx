...
const InprogressOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const inprogressOrders = data.filter(
        (order) => order.status === "Confirmed" || order.status === "Processing" || order.status === "Preparing",
      )
  ...
  const handleMarkReadyForShipment = async (orderId) => {
    try {
      setProcessingAction(true)
      const token =
        localStorage.getItem("adminToken") || localStorage.getItem("token") || localStorage.getItem("authToken")
      ...
      setOrders(orders.map((order) => (order._id === orderId ? { ...order, status: "Ready for Shipment" } : order)))
      showToast("Order marked as ready for shipment!", "success")
      setProcessingAction(false)
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="ml-64 p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">In Progress Orders</h1>
            <p className="text-gray-600 mt-1">Orders currently being prepared and processed</p>
          </div>
          <button
            onClick={fetchOrders}
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="text-sm text-gray-600">
            In Progress: <span className="font-semibold text-blue-600">{filteredOrders.length}</span>
          </div>
        </div>
                  ...
                  {filteredOrders.map((order) => (
                    <tr key={order._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        <button
                          onClick={() => handleMarkReadyForShipment(order._id)}
        ...
        {filteredOrders.length === 0 && !loading && (
          <div className="text-center py-12">
            <div className="text-gray-500 text-lg">No orders in progress</div>
            <p className="text-gray-400 mt-2">Orders being processed will appear here</p>
          </div>
        )}
      </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <button
                      onClick={() => handleMarkReadyForShipment(selectedOrder._id)}
